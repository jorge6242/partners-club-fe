...
import { update, create, get, getAll as getCardPersons } from "../../actions/cardPersonActions";
...
const CardPersonForm: FunctionComponent<CardPersonFormProps> = ({ id, personId }) => {
    ...
    const { loading } = useSelector((state: any) => state.cardPersonReducer);
    const { banks } = useSelector((state: any) => state.bankReducer);
    const { list: cardTypeList } = useSelector((state: any) => state.cardTypeReducer);
    const { list: cardPersonList } = useSelector((state: any) => state.cardPersonReducer);
    ...
    const handleForm = (form: object) => {
        if (id) {
            dispatch(update({ id, ...form, people_id: personId }));
        } else {
            if (cardPersonList.length < 3) {
                dispatch(create({ ...form, people_id: personId }));
            } else {
                    ...
                    <Grid container spacing={3}>
                        ...
                        <Grid item xs={6}>
                            <CustomSelect
                                ...
                                required
                                register={register}
                                errorsMessageField={errors.card_type_id && errors.card_type_id.message}
